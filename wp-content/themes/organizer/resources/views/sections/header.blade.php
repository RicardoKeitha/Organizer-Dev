...
<header class="banner absolute top-0">
  <nav class="nav-primary mt-6" aria-label="{{ wp_get_nav_menu_name('primary_navigation') }}">
    <div class="w-screen px-3 mx-auto">
      <div class="flex justify-between h-16 w-full md:w-3/4 mx-auto text-white">
        <div class="basis-1/2 md:basis-2/12 my-auto">
          <img src="{{the_field('logo_header','option')}}"/>
        </div>
        <div class="hidden md:block basis-6/12 my-auto">
          @if (has_nav_menu('primary_navigation'))
          {!! wp_nav_menu(['theme_location' => 'primary_navigation', 'menu_class' => 'nav', 'echo' => false]) !!}
          @endif
        </div>
        <div class="basis-1/2 md:basis-4/12 flex-end my-auto flex md:block">
          ...
          <button class="border rounded-lg border-main-pink bg-main-pink py-2 px-4 m:mr-4">
